use visionctl::agent::Agent;
use visionctl::config::Config;
use visionctl::debugger::StateStore;
use visionctl::server::DebugServer;
use std::sync::Arc;
use tokio;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let agent = Agent::new(llm_config)?
        .with_observer(state_store);
    ...
    println!("Agent finished: success={}, message={}", result.success, result.message);
